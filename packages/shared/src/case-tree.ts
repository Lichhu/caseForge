...
export function isPlaceholderCaseTitle(title: string | undefined | null) {
  ...
  if (/^案例详情\s*\[(正|反|正向|反向|异常|边界|接口|权限|端到端|UI|并发)\]\s*$/u.test(
    (title ?? '').trim(),
  )) {
    return true;
  }
...
export function getCaseDisplayTitle(
  ...
  if (!base && requirementTitle) {
    base = buildFallbackCaseTitle(requirementTitle, polarity || DEFAULT_CASE_NATURE);
  }
  if (base && polarity) {
  ...
  const walk = (
    ...
    if (node.kind === 'case' || node.kind === 'scenario') {
      ...
      if (!injectMissingCaseTitle) {
        nextChildren = nextChildren.filter((child) => !isAutoGeneratedCaseTitleNode(child));
...
function findRootNodeByTitle(tree: CaseTreeNode, rootTitle: string): CaseTreeNode | null {
...
function ensureRequirementNodeByPath(tree: CaseTreeNode, path: CaseExcelRowPath): CaseTreeNode | null {
  ...
  const moduleNode = ensureChildByKindAndTitle(systemNode, 'module', moduleTitle);
  return ensureChildByKindAndTitle(moduleNode, 'requirement', requirementTitle);
}

function findRequirementNodeByPath(tree: CaseTreeNode, path: CaseExcelRowPath): CaseTreeNode | null {
  let found: CaseTreeNode | null = null;
  const walk = (
    node: CaseTreeNode,
...
export function addCaseRowToTree(
  ...
  const requirementNode =
    findRequirementNodeByPath(clone, path) || ensureRequirementNodeByPath(clone, path);
  if (!requirementNode) {
    return null;
  }
  const caseNodeId = cryptoRandomId();
  const caseName = input?.caseName?.trim() || input?.caseTitle?.trim() || '新案例';
...
export function removeCaseFromTree(tree: CaseTreeNode, caseNodeId: string): CaseTreeNode | null {
  const clone = cloneCaseTree(tree);
