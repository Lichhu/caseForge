...
## 更新摘要
**变更内容**
- 新增生产环境部署配置：PM2 进程管理、Nginx 反向代理配置
- 添加企业级部署指南，支持 CentOS 内网环境部署
- 提供两种部署方案：Express + PM2（无 Nginx）和 Nginx + PM2
- 增强容器化部署方案的完整性和可执行性
...
**更新** 新增了生产环境部署配置，包括 PM2 进程管理、Nginx 反向代理配置和企业级部署指南，支持 CentOS 内网环境和多种部署模式。
...
- **新增** 生产环境部署配置：PM2 进程管理、Nginx 反向代理配置、部署指南。
...
subgraph "部署配置"
DeployDir["deploy/<br/>部署指南与配置"]
NginxConf["nginx.conf<br/>反向代理配置"]
EcoConfig["ecosystem.config.js<br/>PM2 进程管理"]
end
subgraph "共享包"
...
- [apps/api/package.json:1-82](file://apps/api/package.json#L1-L82)
...
- [deploy/README.md:1-185](file://deploy/README.md#L1-L185)
- [deploy/nginx.conf:1-94](file://deploy/nginx.conf#L1-L94)
- [ecosystem.config.js:1-56](file://ecosystem.config.js#L1-L56)
...
- **新增** 生产环境部署组件
  - PM2 进程管理：单实例 fork 模式运行 API，多实例 cluster 模式运行前端静态服务。
  - Nginx 反向代理：高性能静态资源服务和 API 反向代理。
  - Express 内置服务器：无需 Nginx 的轻量级部署方案。
...
- [apps/api/src/config/load-env.ts:1-57](file://apps/api/src/config/load-env.ts#L1-L57)
...
**更新** 新增了两种部署架构：基于 Nginx 的企业级方案和基于 Express + PM2 的轻量级方案。

```mermaid
graph TB
Client["客户端浏览器"] --> Nginx["反向代理/Nginx<br/>域名与SSL终止"]
Client2["客户端浏览器"] --> Express["Express + PM2<br/>轻量级方案"]
Nginx --> LB["负载均衡/健康检查"]
LB --> API1["API 实例 #1<br/>PM2 fork 单实例"]
LB --> API2["API 实例 #2<br/>PM2 fork 单实例"]
LB --> API3["API 实例 #3<br/>PM2 fork 单实例"]
API1 --> DB["数据库/MySQL"]
API2 --> DB
API3 --> DB
...
API2 --> Redis
API3 --> Redis
Express --> WebStatic["前端静态资源<br/>PM2 cluster 多实例"]
Express --> APIProxy["API 反向代理<br/>127.0.0.1:34550"]
```
...
#### PM2 进程配置
- **API 服务**：使用 fork 单实例模式运行，避免 AI 案例生成任务重复处理
- **前端服务**：使用 cluster 多实例模式，充分利用 CPU 资源
- **内存管理**：配置自动重启策略，防止内存泄漏影响服务稳定性
- **日志管理**：统一的日志输出和错误追踪

#### 部署架构选择
CaseForge 提供两种生产部署方案：

1. **Express + PM2 方案**（推荐内网环境）
   - 无需安装 Nginx，简化部署复杂度
   - Express 内置压缩和缓存功能
   - 端口 8080 对外提供服务
   - 适合 CentOS 内网环境

2. **Nginx + PM2 方案**（推荐公网环境）
   - 高性能静态资源服务
   - 完整的反向代理功能
   - 支持 SSL 证书管理
   - 适合生产环境和高并发场景
...
```mermaid
graph LR
subgraph "Express + PM2 方案"
A1["PM2 Cluster<br/>caseforge-web<br/>端口: 8080"] --> A2["Express 静态服务<br/>Gzip 压缩"]
A1 --> A3["API 反向代理<br/>127.0.0.1:34550"]
A3 --> A4["PM2 Fork<br/>caseforge-api<br/>端口: 34550"]
end
subgraph "Nginx + PM2 方案"
B1["Nginx<br/>端口: 80/443"] --> B2["静态资源<br/>apps/web/dist"]
B1 --> B3["API 反向代理<br/>upstream caseforge_api"]
B3 --> B4["PM2 Fork<br/>caseforge-api<br/>端口: 34550"]
end
```
...
- 共享包：提供类型与契约，避免重复定义。
- **新增** 部署依赖：PM2 进程管理、Express 服务器、Nginx 反向代理。
...
WEB --> SharedRef
Deploy["部署配置"] --> PM2["PM2 进程管理"]
Deploy --> Express["Express 服务器"]
...
- 前端
  ...
  - 使用 CDN 加速静态资源。
- **新增** 部署层性能优化
  - **AI 并发控制**：通过 `CASE_GENERATE_CONCURRENCY` 环境变量调整 AI 接口调用并发度（默认 4，最大 32）
  - **PM2 实例配置**：前端使用 `max` 实例数充分利用 CPU，API 保持单实例避免任务重复
  - **Nginx 优化**：启用 gzip 压缩、长期缓存策略、连接池优化
  - **内存管理**：配置 PM2 自动重启阈值，防止内存泄漏影响服务稳定性
...
- **新增** 部署相关问题
  - **PM2 进程状态**：使用 `pm2 status` 检查进程状态，`pm2 logs` 查看详细日志
  - **端口冲突**：确认 8080（Express 方案）或 80/443（Nginx 方案）端口未被占用
  - **权限问题**：CentOS 环境下监听 80 端口需要 root 权限或使用 setcap 授权
  - **防火墙配置**：确保 firewalld 放行相应端口
...
- [deploy/README.md:149-185](file://deploy/README.md#L149-L185)

## 结论
本指南基于仓库现有代码，给出了从容器化到 CI/CD、从监控告警到数据库治理、从负载均衡到 SSL 管理的全栈运维方案。**更新** 新增了生产环境部署配置，包括 PM2 进程管理、Nginx 反向代理和企业级部署指南，支持多种部署模式和内网环境部署。建议在生产环境中配套引入容器编排（Kubernetes/Docker Compose）、CI/CD 平台（GitHub Actions/GitLab CI）、APM/日志/告警平台与自动化备份策略，以实现高可用与可追溯的交付闭环。
...
#### 环境准备
- **Node.js 20+**：推荐使用 NodeSource 或离线安装包
- **PM2**：全局安装进程管理工具
- **防火墙配置**：CentOS 环境下需放行 8080 端口（Express 方案）或 80/443 端口（Nginx 方案）

#### 部署方式选择

**方式一：Express + PM2（推荐内网环境）**
- 优点：无需安装 Nginx，部署简单，内置压缩和缓存
- 适用：CentOS 内网环境，对性能要求不极端的场景
- 端口：8080

**方式二：Nginx + PM2（推荐公网环境）**
- 优点：高性能静态资源服务，完整的反向代理功能，支持 SSL
- 适用：生产环境，高并发场景，需要 SSL 证书管理
- 端口：80/443

#### 环境变量配置
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `PORT` | `8080` | 前端对外端口（Express 方案） |
| `API_TARGET` | `http://127.0.0.1:34550` | 后端 API 地址 |
| `DIST_DIR` | `../apps/web/dist` | 前端构建产物路径 |
| `CASE_GENERATE_CONCURRENCY` | `4` | AI 并发上限（1-32） |
...
#### 性能调优建议
- **AI 并发控制**：根据 AI 接口承受能力调整 `CASE_GENERATE_CONCURRENCY`
- **PM2 实例数**：前端使用 `max` 实例数充分利用 CPU
- **数据库索引**：确保执行索引优化脚本
