...
## 更新摘要
**所做更改**
- 更新了调试运行端点的文档，添加了ignoreSslVerify参数的详细说明
- 新增了SSL证书验证控制的架构说明
- 更新了调试执行的流程图，包含SSL验证选项的处理逻辑
- 增强了故障排查指南，包含SSL相关的错误处理
...
### 调试执行（Debug Run）
- **新增**：调试执行端点支持 `ignoreSslVerify` 参数，用于控制 HTTPS 证书验证行为
- 支持两种调试模式：基于环境的调试和基于步骤地址的调试
- 关键流程：参数验证、环境变量解析、SSL 验证控制、请求执行、结果持久化

**更新** 调试执行端点现在支持 SSL 证书验证控制，可以通过 `ignoreSslVerify` 参数显式指定，或在按步骤地址调试时继承所选环境服务的配置。
...
Client->>Ctrl : "POST /api-test/ : projectId/transactions/ : transactionId/cases/debug-run"
Ctrl->>Ctrl : "验证请求参数"
Ctrl->>ExecSvc : "调用 debugRun(ignoreSslVerify)"
ExecSvc->>EnvSvc : "获取环境服务配置"
alt 按步骤地址调试
ExecSvc->>EnvSvc : "检查服务级 ignoreSslVerify"
EnvSvc-->>ExecSvc : "返回 SSL 验证配置"
end
ExecSvc->>Fetch : "发送 HTTP 请求可选忽略证书验证"
Fetch-->>ExecSvc : "返回响应"
ExecSvc-->>Ctrl : "返回调试结果"
...
图表来源
- [apps/api/src/modules/api-test/controller/api-test.controller.ts:782-833](file://apps/api/src/modules/api-test/controller/api-test.controller.ts#L782-L833)
- [apps/api/src/modules/api-test/service/api-execution.service.ts:709-779](file://apps/api/src/modules/api-test/service/api-execution.service.ts#L709-L779)
- [apps/api/src/modules/api-test/service/api-environment.service.ts:153-163](file://apps/api/src/modules/api-test/service/api-environment.service.ts#L153-L163)
...
**更新** 环境服务相关 DTO 现在包含 `ignoreSslVerify` 字段，允许在环境服务级别配置是否忽略 HTTPS 证书验证。
...
**更新** 系统现在能够正确处理 SSL 证书验证相关的错误，包括自签名证书被拒绝的连接错误。
...
**更新** 系统实现了 SSL 连接池优化，当启用忽略证书验证时，会创建一个共享的 undici Agent 实例，避免频繁的连接池重建开销。
...
**更新** 新增了 SSL 证书验证相关的故障排查指南，帮助解决自签名证书和测试环境中的 HTTPS 连接问题。
...
## 结论
本控制器围绕"交易码—文档—用例—环境—执行集—执行—报告"的完整闭环，提供了清晰的 REST 接口与严格的参数校验。通过 DTO 封装输入、服务层封装业务、控制器统一封装响应，形成高内聚低耦合的架构。**最新增强**包括对 SSL 证书验证的灵活控制，支持在调试和执行过程中根据需要忽略 HTTPS 证书验证，特别适用于自签名证书的测试环境。建议在生产环境中进一步完善异步任务、缓存与监控体系，以提升稳定性与可观测性。
...
**更新** 新增了关于 SSL 证书验证的最佳实践建议，指导在不同环境下合理使用忽略证书验证功能。