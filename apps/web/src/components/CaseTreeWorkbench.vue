        ...
        >
          ...
          <div v-else ref="mindCanvasWrap" class="mind-canvas-wrap">
            <a-alert
              ...
              show-icon
              banner
              message="案例较多时建议在 Excel 视图编辑；导图默认仅展示到测试要点，展开后按需加载案例。"
            />
              ...
              <template v-if="selectedNode">
                ...
                <div class="inspector-children-scroll">
                  ...
                  <template v-else-if="inspectorChildSummaries.length">
                    <label>子节点（{{ inspectorChildSummaries.length }}）</label>
...
function createLazyPlaceholder(
  requirementId: string,
  caseCount: number,
): NodeObj<MindNodeMeta> {
  return {
    id: `lazy:${requirementId}`,
    topic: `${caseCount} 条案例`,
    expanded: false,
    tags: ["待加载"],
    style: nodeStyle("metadata"),
    metadata: {
      kind: "metadata",
    ...
    if (options.fetchedRequirementIds.has(node.id)) {
      ...
      if (!isExpanded) {
        return {
          ...
          children:
            caseCount > 0 ? [createLazyPlaceholder(node.id, caseCount)] : [],
        };
      }
      return {
        id: node.id,
...
function resolveMindNodeExpanded(node: CaseTreeNode, lazyCases = false) {
  ...
  // 案例及以下默认收起；用户手动展开后 collapsed=false
...
function summarizeCaseTreeChild(
  ...
  const parentCase = node.kind === "case" ? node : undefined;
  const nextRequirement =
    node.kind === "requirement"
      ? simplifyRequirementTitleForDisplay(node.title)
      : requirementTitle;
...
async function refreshInspectorChildren(node: NodeObj<MindNodeMeta>) {
  ...
  if (kind === "requirement") {
    const storeNode = store.activeRun?.tree
      ...
      : null;
    if (fetchedRequirementIds.value.has(node.id) && storeNode) {
      inspectorChildSummaries.value = getRequirementCaseChildren(storeNode).map(
        (child) => summarizeCaseTreeChild(child),
      );
      inspectorChildrenLoading.value = false;
      return;
    }
    ...
    try {
      await loadRequirementCases(node.id, { expand: false });
