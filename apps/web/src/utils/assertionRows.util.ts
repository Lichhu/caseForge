...
export const ASSERTION_TYPE_OPTIONS: Array<{ value: AssertionType; label: string }> = [
...
export function createEmptyAssertionRow(protocol: CaseProtocol = 'http'): AssertionRow {
  return {
    id: createRowId(),
    description: '',
...
export function assertionsToRows(assertions: ApiAssertion[] | undefined): AssertionRow[] {
...
export function rowToAssertion(row: AssertionRow): ApiAssertion | null {
  ...
  if (row.expected.trim()) {
    assertion.expected = row.expected.trim();
  }
  return assertion;
}
...
export function showsExpectedField(type: AssertionType | ''): boolean {
  return type !== 'string' && type !== 're' && type !== 'default' && type !== 'rsp_download';
}
